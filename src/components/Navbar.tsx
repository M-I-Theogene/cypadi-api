        ...
        >
          ...
          >
            {links.map((l) => {
              ...
              // On home, use ScrollLink with spy so active styles update on scroll
              if (isHome) {
                return (
                  <ScrollLink
                    key={l.to}
                    to={l.to === "blog" ? "blog" : l.to}
                    spy
                    smooth
                    offset={-80}
                    duration={600}
              ...
              // From inner pages (e.g. /blog/:slug), route back to home with hash.
              // Global hash listener in App.tsx handles the actual scrolling.
              if (l.to === "blog") {
                return (
                  <RouterLink
                    key={l.to}
              ...
              return (
                <RouterLink
                  ...
                  onMouseLeave={magLeave()}
                  onClick={() => {
                    // Close nav; scrolling handled globally via hash listener
                    handleNavClick(l.to);
                  }}
                >
                  {l.label}
